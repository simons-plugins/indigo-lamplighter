...
- `presence_variables`: Indigo variable ids that count as presence inputs
  alongside `presence_devices`, same any-of semantics. A variable reads ON
  when its value, lower-cased and stripped, is one of `true` / `on` / `yes` /
  `1` / `home`, and OFF for anything else, including empty -- how the house's
  phone-presence variable (`SimonHome`, set to the string "true"/"false")
  becomes a presence input without a shadow relay device.
...
- Three pieces of state, and two are persisted:
  ...
  - `confirmed_vacant_last_seen` — the `last_seen` value a real evaluation has
    already judged the hold expired for. Also persisted (R13, issue #15): the
    other piece a restart must not lose, or a boundary crossed just after
    start-up can revive an expired hold exactly like the un-patched bug did.
    See the next bullet.
...
- A period may set its own `hold_seconds`, overriding the zone's for as long
  ...
  shorten a hold that is still running (R4). Once a sighting's hold has been
  judged expired and the zone has gone VACANT, it is never revived by a later
  period handing it a longer hold against that same sighting** (issue #15,
  2026-09-15: the Dining Room's 900 s Early hold expired at 07:42 with
  nobody back, and the 08:00 boundary into Working Day's 3600 s hold turned
  the lights on for an empty room). Only a fresh sighting after the VACANT
  transition, or live presence, occupies the zone again.
