...
- **Version bump per PR**: `Info.plist` `PluginVersion` (`YYYY.R.P`); `CFBundleVersion` stays `1.0.0`.
  **Pre-release rule (Simon, 2026-09-05, applied until the first release, v2026.1.12 on 2026-09-06): the build up to the first release was `2026.1.x` — PATCH bumps only, so the first shipped version was not `2026.17`.** **Post-release (Simon, 2026-09-07): the normal workspace rule — patch for fixes/docs/internal, minor for user-visible changes.**
